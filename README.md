...
统一处理 OverseaVideo（港澳台）、BStar（国际版）、PCDN（中国大陆）与 MCDN（中国大陆）请求，并按对应模块参数选择重定向目标。
...
设置请求同时匹配 `biliverse.github.io` 与 `app.bilibili.com` 的 `/configs/Redirect` 和 PreferencePanes 通用路径。官方域名仅作为本机代理映射地址，配置仍由本模块的同版 Gist / Release 提供。主页面与公共静态资源映射仅由 Enhanced 安装，其他模块不管理主入口。
